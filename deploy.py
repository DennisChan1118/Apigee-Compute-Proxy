...
import zipfile
# ssl module is required for skip ssl check (insecure)
import ssl

def httpCall(verb, uri, headers, body):
    if httpScheme == 'https':
        # default from apigee, but it will check the SSL certificates, not work for Nextbank SSL cert without working domain name.
        #conn = httplib.HTTPSConnection(httpHost)
        # rewrited as following line to skip ssl check (insecure)
        conn = httplib.HTTPSConnection(httpHost,timeout=5, context=ssl._create_unverified_context())
    else:
